...
function advance() {
    ...
    while (state.flowIndex < flow.length) {
        ...
        if (nextId === 'leadCapture' && (state.flow === 'build') && state.origem === 'ia') {
            const link = getLink('/primeiro-cliente-av-2', 'primeiro-cliente-av-build-vsl-ia-2');
            document.getElementById('step-content').innerHTML = `
...
function gerarResultado() {
    ...
    if (state.flow === 'explore') {
        ...
        <p>É o passo certo pra quem quer sair da pesquisa e começar pra valer.</p>`,
                btn: makeCTA('👉 Conhecer o programa', '/primeiro-cliente-av-3', 'primeiro-cliente-av-explore-vsl-ia-3'),
            };
        }
