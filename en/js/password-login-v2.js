...
function redirectToNext() {
    ...
    $("#sign_up").hide();
    $("#loading").html(
      '<div><h1>Login successful!</h1><br /><p>Redirecting automatically...</p></div>'
    );
    $("#loading").show();
  ...
  function updateLinks() {
    ...
    $("#reset-link").each(function () {
      const $button = $(this);
      const url = new URL('./request-new-password', window.location);
  ...
  $(submitBtn).on('click', async (event) => {
    ...
    // Log in
    try {
      const res = await fetch(`${window.apiURL}/login?state=${window.role}&en=1`, {
        method: "POST",
        credentials: "include",
        body: JSON.stringify({
          username: $("#email").val(),
